...
Implements Command
	...
	#tag Method, Flags = &h0
		Sub execute()
		  ...
		  If ccUI.inputField(ccUI.formFieldList.Value(kCMD_PROJECT_NAME)).Text<>"" Then
		    //load data into dictionary
		    For i=1 To ccUI.maxLabelCount
		      projectData.Value(ccUI.inputField(i).Tag)=ccUI.inputField(i).Text.Trim
		    Next i
		    
		    //create new project database and update projects list
		    If SabreDatabaseManager.ProjectNew(projectData) Then
		      Dim cm As Command
